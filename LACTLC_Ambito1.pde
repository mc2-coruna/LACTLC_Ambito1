PImage Fondo;
int Temporizador;
PFont TipoLetraPregunta;
PFont TipoLetraSiNo;
PFont TipoLetraTexto;
...
String TemaIntruso;
String[] Preguntas = {
  "¿Cuál de estos anillos es de ", 
  "¿Cuál de estos asteroides es ", 
  "¿Cuál de estos cometas es el ", 
  "¿Cuál de estos planetas es ", 
  "¿Cuál de estos planetas enanos es ", 
  "¿Cuál de estos satélites es ", 
  "¿Cuál de estas imágenes del Sol es ", 
  "¿Cuál de estas imágenes de la Tierra "};
String RaizPregunta;
...
String[] NombresTierra = {"son cumulonimbos", "es el Canal de la Mancha", "es Namibia"};
...
void setup() {
  ...
  background(100);
  image(Fondo,0,0);
  TipoLetraPregunta = createFont("Bourgeois Bold Condensed.ttf",48);
  TipoLetraSiNo = createFont("Bourgeois Bold Condensed.ttf",72);;
  TipoLetraTexto = createFont("Bourgeois Med.ttf",36);;
  EscogerPregunta();
}

void draw() {
  // para cuando la peña no haga nada
  if (millis() - Temporizador > 60000) {
    Temporizador = millis();
    EscogerPregunta();
  }
}
...
void ShowMeTheFotos() {
  ...
  rect(5+(OrdenEnPantalla[0])*605,30,604,604);
  image(Imagen1, 7+(OrdenEnPantalla[0])*605, 32);

  Imagen2 = loadImage(Archivo2);
  //text(Archivo2, 30+(OrdenEnPantalla[1])*630, 170);
  rect(5+(OrdenEnPantalla[1])*605, 30,604,604);
  image(Imagen2, 7+(OrdenEnPantalla[1])*605, 32);

  Imagen3 = loadImage(Archivo3);
  //text(Archivo3, 30+(OrdenEnPantalla[2])*630, 170);
  rect(5+(OrdenEnPantalla[2])*605,30,604,604);
  image(Imagen3, 7+(OrdenEnPantalla[2])*605, 32);
  
  textFont(TipoLetraPregunta);
  fill(0);
  textAlign(CENTER, CENTER);
  text(PreguntaCompleta, 960, 675);
...
void HasGanao() {
  ...
  image(Imagen1, 7, 32);
   ...
   textFont(TipoLetraSiNo);
  fill(150);
  textAlign(LEFT, CENTER);
  text(Bravos[RandoMizador], 640,75);
  fill(0);
  textAlign(LEFT, CENTER);
  textFont(TipoLetraTexto);
  text(ExplicacionAMostrar,640,15,1100,500);
  textSize(18);
  text("Pulsa un botón para seguir",640,615); 
  ElPulsado = -1;
}
...
void Nope() {
  ...
  fill(150);
  textAlign(CENTER, CENTER);
  text(Nones[RandoMizador], 960, 800);
  ElPulsado = -1;
}